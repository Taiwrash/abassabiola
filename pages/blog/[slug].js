import { ButtonType, PageType, SubscribeSize } from '../../lib/types.ts';
...
import generateSocialImage from '../../lib/generateSocialImage.ts';
...
const ArticlePage = ({
  ...
  const publishedOn = data.publishedOn
  ...
  return (
    <Container
      ...
      title={`${data.title} - Abass Abiola`}
      description={data.summary}
      imageUrl="/assets/Home Page.jpg"
      date={publishedOn}
        ...
        <article className="col-span-9 mt-12">
          <div className="space-y-12">
            ...
            <div className="flex items-center justify-between space-x-4">
              ...
              <div className="md:hidden">
                <ShareArticle title={data.title} slug={data.slug} />
              </div>
            </div>
...
export async function getStaticPaths() {
  ...
  // dont get paths fo r cms posts, instead, let fallback handle it
...
export async function getStaticProps({ params}) {
  ...
  const { content, data, slug} = matter(postFile)
  // const mdxSource = await serialize(content, { scope: data }) 
  return { props: { data , content}, revalidate: 30 }
}

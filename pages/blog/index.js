...
import { SubscribeSize } from '../../lib/types.ts';
import { Tag } from '../../components/Tag';
import { handleArticleClicked } from '../../lib/handleArticleClick';
// import siteMetadata from '@/data/siteMetadata';
import slugify from 'slugify';
import {post as postsFromCMS} from '../../content'
import matter from 'gray-matter';
export default function Blog({ featuredArticle, articles }) {
  const [selectedTag, setSelectedTag] = useState('');
  const [searchValue, setSearchValue] = useState('');

  const filteredArticles = articles
    .sort((a, b) => Number(new Date(b.publishedOn)))
    .filter((post) => {
      return (
  ...
  return (
    <Container title="Blog - Abass Abiola">
      <h1>
        ...
        >
          <div>
            <Image
              ...
              placeholder="blur"
              blurDataURL={featuredArticle.image}
              width={1200}
              height={684}
              layout="intrinsic"
              alt={'article cover'}
            ...
            <div className="">
              <div className="flex items-center mt-4 md:justify-start">
                ...
                <span className="flex w-3 h-3">
                  <span className="relative inline-flex w-3 h-3 bg-purple-900 rounded-full"></span>
                  <span className="absolute inline-flex w-3 h-3 bg-purple-800 rounded-full opacity-75 animate-ping"></span>
                </span>
              </div>
